...
import { MoreVertical, Clock, Calendar, BadgeDollarSign, Check, X } from "lucide-react"
import { type ApplicationsType, ApplicationStatus } from "@/types/applications.types"
    ...
    <TableRow key={job.id}>
      ...
      {visibleColumns.includes("company") && (
        <TableCell className="px-2 sticky left-[40px] bg-background z-20">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="font-medium truncate max-w-[100px]">{job.company || "N/A"}</div>
          </div>
        </TableCell>
      )}
      ...
      {visibleColumns.includes("role") && (
        <TableCell className="px-2 sm:px-4">
          <div className="flex flex-col">
            <span className="font-medium truncate">{job.role || "N/A"}</span>
            <span className="text-xs text-muted-foreground hidden sm:block">{job.city}</span>
          </div>
        </TableCell>
      )}
      ...
      {visibleColumns.includes("salary") && (
        <TableCell className="hidden md:table-cell">
          <span className="font-medium">{job.ctcOffered || "N/A"}</span>
        </TableCell>
      )}
      ...
      {visibleColumns.includes("status") && (
        <TableCell className="hidden md:table-cell">
          <Badge
            variant={STATUS_COLOR_MAP[job.status] === "success" ? "default" : "outline"}
            className={`${getStatusBadgeClass(job.status)} capitalize w-fit inline-flex items-center`}
          >
          ...
          <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent align="end">
              ...
              <div className="border-t">
                <JobApplicationDetailsPopup selectedApplication={job} />
              </div>
            </DropdownMenuContent>
          </DropdownMenu>
        </TableCell>
