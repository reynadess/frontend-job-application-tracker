...
import type { JobTableFilters, JobTableSort, JobTablePagination } from "../types"
...
export function useJobTable() {
  ...
  const deleteApplicationById = useApplicationsStore((state) => state.deleteApplicationById)
  ...
  const filteredAndSortedJobs = useMemo(() => {
    const filtered = filterApplications(Applications, filters.search, filters.status)
    return sortApplications(filtered, sort)
  }, [Applications, filters.search, filters.status, sort])

  const paginatedJobs = useMemo(() => {
    return paginateData(filteredAndSortedJobs, pagination.currentPage, pagination.rowsPerPage)
  }, [filteredAndSortedJobs, pagination.currentPage, pagination.rowsPerPage])

  const totalPages = Math.ceil(filteredAndSortedJobs.length / pagination.rowsPerPage)
  ...
  const handlePaginationChange = (newPagination: Partial<JobTablePagination>) => {
  ...
  const toggleAllJobs = () => {
    setSelectedJobs(selectedJobs.length === paginatedJobs.length ? [] : paginatedJobs.map((job) => job.id))
  }
