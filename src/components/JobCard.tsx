...
const JobCard: React.FC<JobCardProps> = ({ job }) => {
  ...
  const getExperienceStyles = (experience: Job['experience']) => {
    const variants = {
      ...
      mid: {
        badge:
          'dark:bg-blue-900/50 dark:text-blue-300 bg-blue-100 text-blue-800',
      ...
      <div className="flex flex-wrap gap-2">
        {job.skills.slice(0, 3).map((skill, index) => (
          <Badge
            key={index}
            variant="outline"
            className="bg-gray-100 text-xs text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"
          >
            {skill}
          </Badge>
        ))}
